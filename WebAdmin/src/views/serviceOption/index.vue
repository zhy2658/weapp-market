...
<el-select v-model="status"  @change="selectPlatform"  >
  <el-option  v-for="(item,index) in memberGradeOption()" :key="index" :value='index' :label='item'>{{item}}</el-option>
</el-select>
<!-- {{memberGradeOption}} -->
  ...
  <Dialog v-model="dialogVisible"  @initOrdereList="initOrdereList"  :id="id" :currentRow="currentRow" :status="status" />
