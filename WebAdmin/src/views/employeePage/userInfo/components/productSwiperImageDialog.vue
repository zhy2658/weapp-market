<template>
  ...
  >
    ...
    <el-form ref="formRef" :model="form" label-width="100px" style="text-align: center" :rules="rules">
      ...
      <el-upload :disabled="tableData.length>=4"
        :headers="headers"
        class="avatar-uploader"
        :action="getServerUrl()+'/users/uploadUserImgs'"
...
const initProductSwiperImageList=async()=>{
  console.log('xxx')
  const res=await axios.get("product/listImg/"+form.value.productId);
...
const handleConfirm = () => {
  ...
  formRef.value.validate(async (valid) => {
    if (valid) {
      console.log("xxx")
      let result=await axios.post("product/addImg",form.value);
...
const handleDelete = (id) => {
  ...
  ElMessageBox.confirm(
    ...
    .then(async() => {
      console.log("id="+id)
      let res=await axios.get("product/deleteImg/"+id);
      if(res.data.code==0){
        ElMessage({
          type: 'success',
          message: '删除成功！',
